...
# بازآرایی‌ها
## الگوی Facade: مورد اول
در این مورد ما این الگو را برای کلاس `Parser` اعمال کردیم. برای این کار کلاس `ParserFacade` را اضافه کردیم. همچنین exception handling را به جای کلاس `Main` در کلاس `ParserFacade` انجام دادیم. کلاس `Main` را نیز متناسب با آن تغییر دادیم.

## الگوی Facade: مورد دوم
در این مورد ما این الگو را برای کلاس `CodeGenerator` اعمال کردیم. برای این کار کلاس `CodeGeneratorFacade` را اضافه کردیم. از این کلاس تنها متد `semanticFunction` استفاده می‌شد پس تنها آن را در کلاس `CodeGeneratorFacade` آوردیم. همچنین این متد تنها در کلاس `Parser` استفاده می‌شد پس آن را نیز متناسب با الگوی جدید تغییر دادیم. همچنین exception handling را به جای کلاس `Parser` در کلاس `CodeGeneratorFacade` انجام دادیم.

## الگوی Replace Type Code with State/Strategy
این مورد را به روی کلاس `Address` اعمال کردیم. برای این کار ابتدا `TypeAddress` که یک enum بود را حذف و یک کلاس abstract با همان نام ساختیم که در آن کلاس متد `toString` را به صورت abstract وجود اضافه کردیم. سپس سه کلاس `Direct` و `Indirect` و `Imidiate` را اضافه کردیم که از `TypeAddress` ارث‌بری می‌کنند. هر کدام متد `toString` را با توجه به کلاسشان پیاده می‌کنند. حال در کلاس `Address` تنها متد `toString` را به روی متغیر `Type` صدا می‌کنیم. همچنین یک سری تغییرات (ایجاد کلاس از هر نوع به جای پاس دادن آن نوع) در کلاس `CodeGenerator` می‌دهیم تا با تغییرات جدید سازگار شود.

## الگوی Separate Query from Modifier
این مورد را به روی کلاس `Memory` اجرا می‌کنیم. در این کلاس به جای متد `saveMemory` دو متد `addMemory` و `getMemorySize` را اضافه می‌کنیم تا عملیات‌های نوشتن و خواندن جدا شوند. حال جاهایی که متد `saveMemory` را صدا زده بودند شناسایی می‌کنیم و به جای آن دو متد جدید را جایگزین می‌کنیم.

## الگوی Self Encapsulate Field
این مورد را در کلاس `Parser` و به روی متغیر `rules` اجرا می‌کنیم. برای این کار دو متد `getRules` و `setRules` را پیاده‌سازی می‌کنیم. حال به جای دسترسی مستقیم به متغیر `rules`، از آن دو متد استفاده می‌کنیم.

## الگوی دلخواه اول: الگوی Parameterize Method
این مورد را در کلاس `CodeGenerator` اعمال می‌کنیم. اگر توجه کنیم می‌بینیم گه به جای سه متد `add` و `sub` و `mult` می‌توانیم از یک متد مشترک استفاده کنیم چون بدنه‌ی آن‌ها بسیار به هم شبیه هستند. برای این کار متد `basicOperations` را اضافه می‌کنیم و پارامتر `operation` را به آن پاس می‌دهیم.

## الگوی دلخواه دوم: الگوی Encapsulate Field
برای اعمال این الگو کلاس `Action` را انتخاب می‌کنیم. ابتدا سطح دسترسی متغیر `action` را به `private` تغییر می‌دهیم و سپس برای آن یک getter تعریف می‌کنیم. همچنین  کلاس `Parser` را طوری تغییر می‌دهیم که به جای استفاده از این متغیر به صورت مستقیم، از getter آن استفاده شود.
